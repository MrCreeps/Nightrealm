...
import mrcreeps.mods.nightrealm.procedures.RemoveFromSlotProcedure;
import mrcreeps.mods.nightrealm.procedures.InitiateAllRecipesProcedure;
...
public class DarkforgeGUIGui extends NightrealmModElements.ModElement {
	public static HashMap guistate = new HashMap();
	private static ContainerType<GuiContainerMod> containerType = null;

	public DarkforgeGUIGui(NightrealmModElements instance) {
		super(instance, 88);
	...
	private static class ContainerRegisterHandler {
		...
		public void registerContainer(RegistryEvent.Register<ContainerType<?>> event) {
			event.getRegistry().register(containerType.setRegistryName("darkforge_gui"));
		}
	}

	@OnlyIn(Dist.CLIENT)
	public void initElements() {
		DeferredWorkQueue.runLater(() -> ScreenManager.registerFactory(containerType, DarkforgeGUIGuiWindow::new));
	}
	...
	public void onPlayerTick(TickEvent.PlayerTickEvent event) {
		...
		if (event.phase == TickEvent.Phase.END && entity.openContainer instanceof GuiContainerMod) {
			...
			InitiateAllRecipesProcedure.executeProcedure(Stream
	...
	public static class GuiContainerMod extends Container implements Supplier<Map<Integer, Slot>> {
		...
		public GuiContainerMod(int id, PlayerInventory inv, PacketBuffer extraData) {
			...
			this.customSlots.put(2, this.addSlot(new SlotItemHandler(internal, 2, 52, 44) {
				...
				@Override
				public boolean isItemValid(ItemStack stack) {
					return (HallowboneStickItem.block == stack.getItem());
				}
			}));
			int si;
			int sj;
		...
		private void slotChanged(int slotid, int ctype, int meta) {
			if (this.world != null && this.world.isRemote()) {
				NightrealmMod.PACKET_HANDLER.sendToServer(new GUISlotChangedMessage(slotid, x, y, z, ctype, meta));
	...
	private static void handleSlotAction(PlayerEntity entity, int slotID, int changeType, int meta, int x, int y, int z) {
		...
		if (slotID == 3 && changeType == 1) {
			...
			RemoveFromSlotProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
